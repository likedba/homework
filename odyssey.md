...
6. [Тест производительности БД с пулером и без](#Тест-производительности-БД-с-пулером-и-без)
    - [Без пулера](#Производительность-БД-без-пулера)
    - [С пулером](#Производительность-БД-с-пулером)
    - [С пулером на отдельном хосте](#Производительность-БД-с-пулером-на-отдельном-хосте)
7. [Вывод](#Вывод)
...
pgbench -U pgbench -h 10.1.69.170 -p 6432 -c 1800 -T 600 -P 60 pgbench

![результат теста](/images/postgres_clients_test.png)
![результат теста](/images/postgres_clients_test.png)

На скринах видно, что odyssey принимает 1800 коннектов, при этом с БД поддерживает только 80
...
host2 (12 CPU, 63 RAM)
\
нагрузка: pgbench -U pgbench -h 10.1.69.170 -p 6432 -c 80 -T 600 -P 60 pgbench

![результат теста](/images/postgres_clients_test.png)

## Тест 1 - Производительность БД без пулера

Влияние увеничения количества коннектов к БД на производительность.
...
Влияние увеличения количества коннектов к БД с пулером на производительность.
...
Влияние на производительность увеличения количества коннектов к БД через пулер, установленный на другом хосте.
